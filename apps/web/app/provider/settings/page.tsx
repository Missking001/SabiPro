      ...
      {activeTab === 'profile' && (
        <div className="bg-neutral-0 border border-surface-border rounded-card p-5 md:p-6">
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            <div>
              <label className={labelCls} htmlFor="provider-settings-name">Full name</label>
              <input
            ...
            <div>
              <label className={labelCls} htmlFor="provider-settings-email">Email address</label>
              <input
                id="provider-settings-email"
                type="email"
                value={user?.email || ''}
                disabled
            ...
            <div>
              <label className={labelCls} htmlFor="provider-settings-phone">Phone number</label>
              <input
                id="provider-settings-phone"
                type="tel"
                value={phone}
        ...
        <div className="space-y-4">
          <div className="bg-neutral-0 border border-surface-border rounded-card p-5 md:p-6">
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div>
                <label className={labelCls} htmlFor="provider-settings-current-pw">Current password</label>
                <input id="provider-settings-current-pw" type="password" value={currentPw} onChange={(e) => setCurrentPw(e.target.value)} className={inputCls} placeholder="Enter current password" required />
              </div>
              <div>
                <label className={labelCls} htmlFor="provider-settings-new-pw">New password</label>
                <input id="provider-settings-new-pw" type="password" value={newPw} onChange={(e) => setNewPw(e.target.value)} className={inputCls} placeholder="Minimum 8 characters" required />
              </div>
              <div>
                <label className={labelCls} htmlFor="provider-settings-confirm-pw">Confirm new password</label>
                <input id="provider-settings-confirm-pw" type="password" value={confirmPw} onChange={(e) => setConfirmPw(e.target.value)} className={inputCls} placeholder="Repeat new password" required />
              </div>
      ...
      {activeTab === 'notifications' && (
        <div className="bg-neutral-0 border border-surface-border rounded-card p-5 md:p-6">
          ...
          <div className="space-y-5">
            {[
              { id: 'notif-inquiries', label: 'New inquiries', description: 'When a customer sends you an inquiry' },
              { id: 'notif-reviews', label: 'Review alerts', description: 'When a customer leaves a review on your profile' },
              { id: 'notif-payments', label: 'Payment updates', description: 'When a payment is confirmed or disputed' },
              { id: 'notif-payouts', label: 'Payout releases', description: 'When a payout is released to your bank account' },
