...
export function BottomNav() {
  ...
  // Provider or Admin bottom nav
  const isProviderDashboard = pathname === '/provider/dashboard';
  const isProfile = pathname === '/provider/profile';
  const isInquiries = pathname === '/provider/inquiries';
  const isPayments = pathname === '/provider/payments';
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto flex items-center justify-around h-16">
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-0.5 transition-colors ${
            isProviderDashboard ? 'text-primary-base' : 'text-neutral-500 hover:text-primary-base'
          }`}
          id="bottomnav-provider-dashboard"
        >
          <svg className="w-6 h-6" fill={isProviderDashboard ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={isProviderDashboard ? undefined : 1.8}>
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-0.5 transition-colors ${
            isInquiries ? 'text-primary-base' : 'text-neutral-500 hover:text-primary-base'
          }`}
          id="bottomnav-provider-inquiries"
        >
          <svg className="w-6 h-6" fill={isInquiries ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={isInquiries ? undefined : 1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
          </svg>
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-0.5 transition-colors ${
            isPayments ? 'text-primary-base' : 'text-neutral-500 hover:text-primary-base'
          }`}
          id="bottomnav-provider-payments"
        >
          <svg className="w-6 h-6" fill={isPayments ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24" strokeWidth={isPayments ? undefined : 1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.214.19c1.085.965 2.76.965 3.845 0l.214-.19a2.531 2.531 0 000-3.578L12.3 11.23a2.53 2.53 0 010-3.579l.214-.19a2.76 2.76 0 013.845 0l.214.19M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
