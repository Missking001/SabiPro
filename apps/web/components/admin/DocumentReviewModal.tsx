...
function DocumentPreview({ doc, docUrl, onClose }: { doc: string; docUrl?: string; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm" onClick={onClose} />
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 relative z-10 shadow-2xl border border-[#E5E7EB] max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-bold text-[#18181B]">{doc}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        {docUrl ? (
          <div className="bg-[#FAFAF9] rounded-xl p-4 flex items-center justify-center min-h-[300px]">
            {docUrl.match(/\.(png|jpe?g|webp|gif)$/i) ? (
              <img src={docUrl} alt={doc} className="max-w-full max-h-[60vh] object-contain rounded-lg" />
            ) : (
              <iframe src={docUrl} className="w-full h-[60vh] rounded-lg border-0" title={doc} />
            )}
          </div>
        ) : (
          <div className="bg-[#FAFAF9] rounded-xl p-8 flex flex-col items-center justify-center min-h-[200px] text-center">
            ...
            <p className="text-sm font-medium text-[#71717A]">File not uploaded yet</p>
            <p className="text-xs text-[#A1A1AA] mt-1">The provider has listed this document but has not uploaded the actual file.</p>
          </div>
        )}
      </div>
    </div>
...
export function DocumentReviewModal({ provider, onClose, onApprove, onReject }: DocumentReviewModalProps) {
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-2xl max-w-lg w-full p-6 relative z-10 shadow-2xl border border-[#E5E7EB] max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3 mb-6">
            {provider.docs.map((doc, idx) => {
              const docUrl = provider.docUrls?.[idx];
              return (
                <div
                  key={doc}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#A1A1AA] mt-0.5">
                      {docUrl ? 'Uploaded' : 'Not yet uploaded'}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setPreviewDoc({ name: doc, url: docUrl })}
      ...
      {/* Document Preview */}
      {previewDoc && (
        <DocumentPreview doc={previewDoc.name} docUrl={previewDoc.url} onClose={() => setPreviewDoc(null)} />
      )}
    </>
  );
}
